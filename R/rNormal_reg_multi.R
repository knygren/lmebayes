#' Normal Regression with Multiple Responses
#' @family simfuncs
#' @description
#' Normal-prior regression with a **matrix** response: run
#' \code{\link{rNormal_reg}} once per column of \code{y}, sharing the
#' same design matrix \code{x} (as \code{lm} with \code{cbind} responses).
#'
#' Same argument list as \code{\link{rNormal_reg}} except \code{prior_list}:
#' a **list** of length \code{ncol(y)}; element \code{j} is the
#' \code{prior_list} passed to \code{\link{rNormal_reg}} for column
#' \code{j} of \code{y} (each with \code{mu} and \code{Sigma} or \code{P};
#' optional \code{dispersion} for Gaussian models).
#'
...
#'   \code{\link{rNormal_reg}} output (class \code{"rglmb"}) for the
...
#' @seealso \code{\link{rNormal_reg}}, \code{\link{rNormalGamma_reg_multi}}
#' @example inst/examples/Ex_rNormal_reg_multi.R
#' @export
rNormal_reg_multi <- function(n,
                              y,
                              x,
                              prior_list,
                              offset = NULL,
