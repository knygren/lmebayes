# Smoke test: rNormalGamma_reg_multi on iris (n = 200, l1 = 4, p = 3)
...
prior_list <- lapply(seq_len(ncol(y)), function(j) {
  ...
  list(mu = as.numeric(ps$mu), Sigma = ps$Sigma_0, shape = ps$shape, rate = ps$rate)
})
