# Regression test: glmerb on bayesrules::big_word_club (Gaussian, random slopes).
#
# Migrated from inst/examples/Ex_glmerb.R. Compares lmer coef(), Block 1 prior
# mean mu_all, and posterior means of glmerb draws (averaged over draw within
# each school level).
#
#   Rscript data-raw/test_glmerb_big_word_club.R
#   Rscript data-raw/test_glmerb_big_word_club.R quick   # n = 50 draws
...
## glmerb: school random effects on bayesrules::big_word_club
##
## Compare factor-level means: lmer coef(), Block 1 prior mean mu_all,
## and posterior means of glmerb draws (averaged over draw within each level).
##
...
## glmerb: posterior mean at each grouping-factor level (mean over draw)
...
cat("\nFactor-level means: lmer (raw coef), lmer_full, mu_all, glmerb:\n")
show_cols <- c(
  grp_col,
