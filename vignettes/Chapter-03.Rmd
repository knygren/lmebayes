...
```{r Plant_Prior}
ps = Prior_Setup(
  weight ~ group, data = dat, family = gaussian(), pwt = 0.01,
...
sumlm$coefficients
```
The groupTrt estimate from the classical model is -0.371 with a two sided p-value of 0.2490 (which implies that the one sided p-value real 0.1245).



```{r Plant_lmb_call1}
lmb.D9=lmb(weight ~ group,dNormal(mu,V,dispersion=disp_ML),n=10000)
...
```{r Likelilihood_mean}
pnorm(-0.371/0.3114349)
pt(-0.371/0.3114349,18)
```
